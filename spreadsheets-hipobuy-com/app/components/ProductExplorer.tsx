  ...
  return (
    <section className="explorer wrap" id="finds">
      ...
      <div className="filterBar">
        ...
        <div>{["All", "Hoodies", "Pants", "Jerseys"].map(x => <button className={category === x ? "active" : ""} onClick={() => { setCategory(x); (window as Window & { gtag?: (...args: unknown[]) => void }).gtag?.("event", "category_filter", { category: x, page_path: window.location.pathname }); }} key={x}>{x}</button>)}</div>
