...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const{slug}=await params;const m=articleMeta[slug]||articleMeta["spreadsheet-without-buying-blind"];const url=`/articles/${slug}/`;return{title:m.title,description:m.description,alternates:{canonical:url},openGraph:{title:m.title,description:m.description,url,type:"article",images:[{url:"/superbuy-logo.png",width:756,height:120,alt:"Superbuy spreadsheet research"}]},twitter:{card:"summary_large_image",title:m.title,description:m.description,images:["/superbuy-logo.png"]}}}
