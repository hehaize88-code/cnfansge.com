...
function PageHero({locale,page}:{locale:Locale;page:Exclude<PageKey,"home"|"qcArticle"|"shippingArticle"|"storageArticle"|"orderIdentifiersArticle">}) { const [title,body]=seoPageIntro[locale][page]; return <section className="page-hero"><p className="eyebrow">KAKOBUY VIP / {page.toUpperCase()}</p><h1>{title}</h1><p>{body}</p></section>; }
...
type ArticlePageKey=Extract<PageKey,"qcArticle"|"shippingArticle"|"storageArticle"|"orderIdentifiersArticle">;
const articleKeys:ArticlePageKey[]=["shippingArticle","storageArticle","qcArticle","orderIdentifiersArticle"];
function getArticle(locale:Locale,page:ArticlePageKey){return page==="orderIdentifiersArticle"?orderIdentifiersArticleData[locale]:articleData[locale][page];}
...
function ArticleHighlights({locale}:{locale:Locale}) {
  return <section className="section article-highlights"><div className="section-head"><div><p className="eyebrow">RESEARCH / 04</p><h2>{articleUi[locale].latest}</h2></div><p>{seoPageIntro[locale].articles[1]}</p></div><div className="article-grid">{articleKeys.map((key,i)=>{const a=getArticle(locale,key);return <Link href={localPath(locale,key)} key={key}><span>{a.tag} / 0{i+1}</span><h2>{a.title}</h2><p>{a.lede}</p><b>{a.read}<ArrowRight size={16}/></b></Link>})}</div></section>;
}
...
function ArticlePage({locale,page}:{locale:Locale;page:ArticlePageKey}) {
  ...
  const articleSchema={"@type":"Article",headline:a.title,description:a.lede,datePublished:"2026-08-31",dateModified:"2026-08-31",mainEntityOfPage:canonical,wordCount,author:{"@type":"Organization",name:"Kakobuy VIP Editorial Desk"},publisher:{"@type":"Organization",name:"Kakobuy VIP"}};
  ...
  const faqs=page==="orderIdentifiersArticle"?orderIdentifiersArticleData[locale].faqs:null;
