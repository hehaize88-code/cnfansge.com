...
const articleKeys:Array<Extract<PageKey,"qcArticle"|"shippingArticle"|"storageArticle">>=["shippingArticle","storageArticle","qcArticle"];
...
function ArticleHighlights({locale}:{locale:Locale}) {
  return <section className="section article-highlights"><div className="section-head"><div><p className="eyebrow">RESEARCH / 03</p><h2>{articleUi[locale].latest}</h2></div><p>{seoPageIntro[locale].articles[1]}</p></div><div className="article-grid">{articleKeys.map((key,i)=>{const a=articleData[locale][key];return <Link href={localPath(locale,key)} key={key}><span>{a.tag} / 0{i+1}</span><h2>{a.title}</h2><p>{a.lede}</p><b>{a.read}<ArrowRight size={16}/></b></Link>})}</div></section>;
}
...
function ArticlePage({locale,page}:{locale:Locale;page:Extract<PageKey,"qcArticle"|"shippingArticle"|"storageArticle">}) {
  const a=articleData[locale][page]; const canonical=`https://kakobuyvip.pro${localPath(locale,page)==="/"?"":localPath(locale,page)}`;
  const wordCount=a.sections.flatMap(([,paras])=>paras).join(" ").trim().split(/\s+/).length;
  const schema={"@context":"https://schema.org","@type":"Article",headline:a.title,description:a.lede,datePublished:"2026-08-31",dateModified:"2026-08-31",mainEntityOfPage:canonical,wordCount,author:{"@type":"Organization",name:"Kakobuy VIP Editorial Desk"},publisher:{"@type":"Organization",name:"Kakobuy VIP"}};
