...
function canonicalRedirect(request: Request) {
  ...
  const isApex = url.hostname === "lovegobuys.org";
  const isWww = url.hostname === "www.lovegobuys.org";
  if ((!isApex && !isWww) || (isApex && url.protocol === "https:")) return null;
  url.protocol = "https:";
  url.hostname = "lovegobuys.org";
  url.port = "";
...
function cacheableHtml(response: Response, cacheState: "HIT" | "MISS") {
  ...
  headers.set("X-LoveGoBuy-Cache", cacheState);
...
async function localizeDocumentLanguage(response: Response, url: URL) {
  ...
  const html = (await response.text()).replace(/<html lang="[^"]*">/, `<html lang="${language}">`);
...
const worker = {
  async fetch(request: Request, env: unknown, ctx: ExecutionContext): Promise<Response> {
    ...
    const url = new URL(request.url);
    const shouldCache = request.method === "GET" && url.hostname === "lovegobuys.org" && url.search === "";
    if (!shouldCache) return localizeDocumentLanguage(await handler.fetch(request, env, ctx), url);
    ...
    const response = await localizeDocumentLanguage(await handler.fetch(request, env, ctx), url);
    const contentType = response.headers.get("Content-Type") ?? "";
    if (response.status !== 200 || !contentType.includes("text/html") || response.headers.has("Set-Cookie")) return response;
