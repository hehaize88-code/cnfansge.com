...
import { categoryLabels, localizeProduct } from "@/lib/ui-localization";
...
export function ProductExplorer({ locale, limit }: { locale: Locale; limit?: number }) {
  ...
  const filtered = useMemo(() => products.filter((item) => {
    ...
    return (category === "All" || item.category === category) && `${localized.name} ${localized.category} ${localized.note} ${item.name} ${item.category}`.toLowerCase().includes(query.toLowerCase());
  }
  ...
  return (
    <div className="product-explorer">
      <div className="filter-bar">
        ...
        <div className="filter-scroll" aria-label="Product filters"><SlidersHorizontal size={17} aria-hidden />{productCategories.map((item) => <button className={category === item ? "selected" : ""} key={item} onClick={() => setCategory(item)}>{item === "All" ? t.all : categoryLabels[locale][item]}</button>)}</div>
      </div>
      {filtered.length ? (
        <div className="product-grid">
          {filtered.map((item) => { const localized = localizeProduct(item, locale); return (
            <a className="product-card" href={item.href} target="_blank" rel="nofollow sponsored noopener" key={item.id}>
              <div className="product-image"><Image src={item.image} alt={localized.name} fill sizes="(max-width: 640px) 50vw, (max-width: 1000px) 33vw, 25vw" /></div>
              <div className="product-copy">
                <span className="product-category">{localized.category}</span>
                <h3>{localized.name}</h3>
                <p className="product-note">{localized.note}</p>
