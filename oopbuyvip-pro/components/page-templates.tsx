...
import { articlePath, categories, copy, DESTINATION, faqByLocale, getArticles, sectionPath, type Article, type Locale, type Section } from "@/lib/site";
...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  const websiteLd = { "@context": "https://schema.org", "@type": "WebSite", name: "OOPBUY VIP", url: "https://oopbuyvip.pro", potentialAction: { "@type": "SearchAction", target: `${DESTINATION}/search.html?channelid=2&keywords={search_term_string}`, "query-input": "required name=search_term_string" } };
  return (
    <main className="home-page">
      ...
      <section className="hero">
        <div className="shell hero-inner">
          <div className="hero-copy">
            ...
            <div className="hero-proof"><span><b>08</b> curated routes</span><span><b>05</b> languages</span><span><b>01</b> clear workflow</span></div>
          </div>
          <aside className="hero-collage" aria-label="Current curated products">
            <figure className="collage-main"><Image src="/products/5760.jpg" alt="Curated air-cushion runner" fill priority sizes="(max-width: 720px) 60vw, 32vw" /></figure>
            <figure className="collage-top"><Image src="/products/2450.jpg" alt="Curated web motif hoodie" fill priority sizes="(max-width: 720px) 34vw, 16vw" /></figure>
            <figure className="collage-bottom"><Image src="/products/5841.jpg" alt="Curated mini bag" fill sizes="(max-width: 720px) 32vw, 15vw" /></figure>
            <div className="collage-note"><span>QC</span><b>Look closer<br />before shipping.</b></div>
            <div className="collage-stamp">ROUTES<br />CHECKED<br />SEP 2026</div>
          </aside>
        </div>
      </section>
      ...
      <section className="category-band">
        <div className="shell"><div className="section-head light"><div><p className="kicker">10 ROUTES</p><h2>{t.categories}</h2><p>{t.categoryIntro}</p></div></div>
          <div className="category-grid">{categories.map(([name, path], index) => <a href={`${DESTINATION}${path}`} target="_blank" rel="nofollow sponsored noopener" key={name}><span>{String(index + 1).padStart(2, "0")}</span><b>{name}</b><ArrowUpRight /></a>)}</div>
        </div>
      </section>
...
export function ArticleGrid({ locale, articles }: { locale: Locale; articles: Article[] }) {
  const t = copy[locale];
  const visuals = {
    "oopbuy-beginner-guide-2026": { label: "BEGINNER GUIDE", className: "thumb-guide", Icon: BookOpenCheck },
    "oopbuy-spreadsheet-workflow": { label: "SPREADSHEET", className: "thumb-sheet", Icon: Boxes },
    "oopbuy-qc-photo-checklist": { label: "QC PHOTOS", className: "thumb-qc", Icon: Camera },
    "oopbuy-fees-returns-policy": { label: "FEES & RETURNS", className: "thumb-fees", Icon: RotateCcw },
    "oopbuy-shipping-cost-planner": { label: "SHIPPING", className: "thumb-shipping", Icon: Ruler },
  } as const;
...
export function ArticlePage({ locale, article }: { locale: Locale; article: Article }) {
  const t = copy[locale];
  const articleLd = { "@context": "https://schema.org", "@type": "Article", headline: article.title, description: article.deck, dateModified: "2026-09-01", datePublished: "2026-09-01", author: { "@type": "Organization", name: "OOPBUY VIP Research Desk" }, publisher: { "@type": "Organization", name: "OOPBUY VIP" } };
  return <main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(articleLd) }} /><article className="longform"><header className="article-hero"><div className="shell article-header"><p className="eyebrow">OOPBUY VIP / {t.nav.articles}</p><h1>{article.title}</h1><p>{article.deck}</p><div><span><Clock3 />{article.readTime}</span><span>{t.updated}</span></div></div></header><div className="shell article-layout"><aside><p>IN THIS GUIDE</p>{article.sections.map((section, index) => <a key={section.heading} href={`#section-${index + 1}`}>{String(index + 1).padStart(2, "0")} {section.heading}</a>)}</aside><div className="article-body">{article.sections.map((section, index) => <section id={`section-${index + 1}`} key={section.heading}><span>0{index + 1}</span><h2>{section.heading}</h2>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}<div className="source-box"><PackageCheck /><div><b>{t.sourceNote}</b><p>{t.disclosure}</p>{article.sources && <div className="source-links">{article.sources.map((source) => <a key={source.href} href={source.href} target="_blank" rel="noopener noreferrer">{source.label}<ArrowUpRight /></a>)}</div>}</div></div></div></div></article></main>;
}
