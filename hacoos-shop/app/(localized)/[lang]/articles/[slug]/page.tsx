...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; slug: string }> }): Promise<Metadata> { const { lang, slug } = await params; if (!locales.includes(lang as Locale)) return {}; const locale = lang as Locale; const article = getArticles(locale).find(item => item.slug === slug); if (!article) return {}; return { title: article.title, description: article.deck, alternates: { canonical: `${SITE_URL}${articlePath(locale, slug)}`, languages: alternateUrls("article", slug) }, openGraph: { type: "article", title: article.title, description: article.deck, url: `${SITE_URL}${articlePath(locale, slug)}`, publishedTime: "2026-09-01", modifiedTime: "2026-09-01", images: ["/og.png"] } }; }
